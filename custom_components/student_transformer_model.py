...
from custom_components.student_transformer import StudentTransformer
...
class CustomNLUComponent(GraphComponent):
    ...
    def __init__(self, component_config):
        ...
        self.teacher_weight = component_config['teacher_weight']

        PATH = "./custom_components/trained_models/student_transformer"
        self.path = PATH
        if os.path.exists(PATH) and os.path.exists(f"{PATH}/model.pickle"):
            self.tokenizer = BertTokenizer.from_pretrained(PATH, local_files_only=True)
            model = StudentTransformer()
            settings = torch.load(f"{PATH}/model.pickle")
            to_del_keys = []
            ...
            for key in to_del_keys:
                del settings[key]
            model.load_state_dict(settings)
            model.eval()
            self.model = model
    ...
    def train(self, training_data: TrainingData) -> Resource:
        ...
        bert = BertForSequenceClassification.from_pretrained(
            "./custom_components/trained_models/bert_solo",
            local_files_only=True
        )

        model = StudentTransformer(num_labels=num_labels, bert=bert, teacher_weight=self.teacher_weight, device=device)
        model.cuda()
        ...
        for epoch_i in range(0, epochs):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                logits, loss = model.forward_train(b_input_ids, b_input_mask, b_labels)
        ...
        CustomNLUComponent._save_model("student_transformer", model, tokenizer, intent_list, training_stats)
