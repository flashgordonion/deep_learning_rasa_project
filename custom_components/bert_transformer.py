...
class CustomNLUComponent(GraphComponent):
    ...
    def __init__(self, component_config):
        ...
        PATH = "./custom_components/trained_models/bert_solo"
        if os.path.exists(PATH):
            self.tokenizer = BertTokenizer.from_pretrained(PATH, local_files_only=True)
            self.model = BertForSequenceClassification.from_pretrained(PATH, local_files_only=True)
    ...
    def train(self, training_data: TrainingData) -> Resource:
        ...
        model = BertForSequenceClassification.from_pretrained(
            "bert-base-uncased", num_labels=len(training_data.intents),
        ...
        CustomNLUComponent._save_model("bert_solo", model, tokenizer, intent_list, training_stats)
    ...
    def process(self, messages: List[Message]) -> List[Message]:
        ...
        for message in messages:
            ...
            res = self.model(input_ids, token_type_ids=None, attention_mask=attention_masks)
            logits = res.logits
            prediction = logits.argmax().item()
            soft_pred = self.softmax(logits)
            confidence = soft_pred[:, prediction].item()
